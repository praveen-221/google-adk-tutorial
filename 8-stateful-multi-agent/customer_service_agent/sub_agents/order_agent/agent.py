...
def get_current_time() -> dict:
    """ Get the current timestamp in %Y-%m-%d %H:%M:%S format"""
    return {
        "current_timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    }

def process_refund(tool_context: ToolContext) -> dict:
    """
        Simulates refund process of the AI Marketing Platform course.
    ...
    if course_id not in course_ids:
        return {
            "status": "error", 
            "message": "You don't own this course hence refund is not possible!"
        }

    # Create new list with the course added
    updated_purchased_courses = []
    ...
    for course in current_purchased_courses:
        ...
        if course["id"] == course_id:
            continue
        updated_purchased_courses.append(course)

    # Update purchased courses in state via assignment
    tool_context.state["purchased_courses"] = updated_purchased_courses
...
order_agent = Agent(
    ...
    instruction = """
        You are the order agent for online EdTech platform.
        ...
        When users request a refund:
            ...
            2. If they own it:
                - Use the process_refund tool to process the refund
        ...
        If the user asks about anything else, you should delegate the task to the customer service agent.
    """,
    tools = [get_current_time, process_refund]
)