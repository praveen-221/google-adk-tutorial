...
def get_capital_city(country: str) -> Dict[str, str]:
    ...
    """
    print(f"Executing get_capital_city tool with country: {country}")
    ...
    print(f"Tool response: {{'capital': '{result}'}}")
...
def tool_post_execute(
    ...
    if tool_name == "get_capital_city" and "washington" in original_result.lower():
        ...
        modified_response["capital"] = (
            f"{original_result} (Note: Added by tool callback.)"
        )
...
root_agent = LlmAgent(
    ...
    before_tool_callback = tool_pre_execute,
    after_tool_callback = tool_post_execute,
)